...
- `src/components/`: focused header, hero/discovery, products, experiences/editorial, journey/footer, overlay components, and the authenticated account dashboard demo.
...
The account panel offers a local demo login and a responsive dashboard with overview, orders, wishlist, addresses, returns, and profile/preferences views. No credentials or personal information are requested, and the account data is UI-only demo state. The store is a separate React experience at `/store`, with product catalog, configurable merchandising collections rendered one after another, a full catalog with numbered pagination, search, category/brand/price/availability filters, sorting, product detail pages, variants, galleries, specs, manuals, warranty and return conditions, wishlist, persistent anonymous cart, voucher `SKY10`, AvaStar coin redemption, checkout, saved addresses, online/SnappPay payment simulations, receipts, orders, reviews, return requests, and UI-only AI/support chat. The store also links to Tours, Education, and Magazine. Search filters destination suggestions locally and explicitly describes future unified search. Chat presents clearly unavailable AI and customer-support options without simulating a conversation. All future links retain their intended route URLs but intercept clicks to show an accessible native modal dialog. Escape closes dialogs; focus is contained and returned to the trigger.
...
This phase implements the homepage plus a professional storefront and customer purchasing demo. Real authentication, payment gateways, shipment creation, refunds, CMS/API content, legal pages, contact channels, and social accounts remain unconnected. The store deliberately labels simulated payments, sample data, policies, and content. Replace preview data and generated generic product visuals with approved catalog/content assets before commerce launch. Configure the public origin and an absolute Open Graph image URL when a deployment domain exists.
...
The existing CSS/Intersection Observer stack uses one frame-coalesced scene updater for hero depth, tour-image movement, and an editorial crop crossfade, including browsers without native scroll timelines. On spacious desktop screens the hero briefly holds below the masthead as the sky zooms in. On spacious desktop screens, the existing magazine image briefly stays in place as its copy passes beside it; mobile, short screens, and reduced-motion mode use the normal document flow. No wheel interception or scroll snapping is used.

Language and theme controls crossfade the current viewport with the View Transition API when supported. Reduced motion bypasses snapshots and decorative scroll effects. The progress bar uses native scroll timelines where available and a passive, frame-coalesced fallback elsewhere. Horizontal touch swipes change product images in the active reading direction; vertical pan and pinch zoom remain native, and visible previous/next buttons remain available.
